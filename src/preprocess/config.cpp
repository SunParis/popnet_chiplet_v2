...
void Config::fromJson(const std::string& fname) {
    ...
    if (j.contains("random_seed")) {
        this->random_seed_ = j["random_seed"].get<long>();
    }
...
void Config::fromCMD(int argc, char* const argv[]) {
    std::string opt_str = "hA:c:V:B:F:T:r:I:O:R:L:G:m:C:lD:PE";
    ...
    while (true) {
        ...
        case 'L':
            this->link_length_ = std::stod(optarg);
            break;

        case 'T':
            this->sim_length_ = std::stod(optarg);
            break;
        ...
        case 'r':
            this->random_seed_ = std::stol(optarg);
            break;
        ...
        case 'm':
            this->report_period_ = std::stod(optarg);
            break;
...
Config::Config(int argc, char* const argv[])
    : ary_number_(8), cube_number_(2), virtual_channel_number_(2), physical_port_number_(0),
      in_buffer_size_(64), out_buffer_size_(4), flit_size_(4), link_length_(1000),
      sim_length_(1000000000), trace_fname_(), routing_alg_(0), vc_share_(VCShareType::SHARE),
      topo_file_path_(), report_period_(REPORT_PERIOD_), reconfig_file_path_(), packet_loss_(false),
      end_with_minus_1_(false), delay_fname_(), log_fname_(), sync_protocol_enable_(false),
      random_seed_() {
    std::string help(
        "h: help\n?: help\nA: array size\nc: cube dimension\nV: virtual channel number\nB: buffer "
        "size\nO: outbuffer size\nF: flit size\nL: link legnth\nT: simulation length\nI: trace "
...
void Config::validate() const {
    if (this->ary_number_ <= 0) {
        throw std::invalid_argument("vertices must be greater than zero");
    }
    if (this->cube_number_ <= 0) {
        throw std::invalid_argument("dimension must be greater than zero");
    }
    if (this->virtual_channel_number_ <= 0) {
        throw std::invalid_argument("vc_cnt must be greater than zero");
    }
    if (this->in_buffer_size_ <= 0 || this->out_buffer_size_ <= 0) {
        throw std::invalid_argument("input_buffer and output_buffer must be greater than zero");
    }
    if (this->flit_size_ <= 0) {
        throw std::invalid_argument("flit_size must be greater than zero");
    }
    if (!std::isfinite(this->link_length_) || this->link_length_ < 0) {
        throw std::invalid_argument("link_length must be finite and non-negative");
    }
    if (!std::isfinite(this->sim_length_) || this->sim_length_ < 0) {
        throw std::invalid_argument("time must be finite and non-negative");
    }
    if (!std::isfinite(this->report_period_) || this->report_period_ <= 0) {
        throw std::invalid_argument("report_period must be finite and greater than zero");
    }
    if (this->routing_alg_ < static_cast<long>(RoutingType::XY) ||
        this->routing_alg_ > static_cast<long>(RoutingType::RECONFIGURABLE_GRAPH_TOPO)) {
        throw std::invalid_argument("routing_algorithm must be in [0, 5]");
    }
    if (this->trace_fname_.empty()) {
        throw std::invalid_argument("trace_file must not be empty");
    }
    if ((this->getRoutingAlg() == RoutingType::GRAPH_TOPO ||
         this->getRoutingAlg() == RoutingType::RECONFIGURABLE_GRAPH_TOPO) &&
        this->topo_file_path_.empty()) {
        throw std::invalid_argument("topology_file is required for graph routing");
    }
    if ((this->getRoutingAlg() == RoutingType::GRAPH_TOPO ||
         this->getRoutingAlg() == RoutingType::RECONFIGURABLE_GRAPH_TOPO) &&
        this->cube_number_ != 1) {
        throw std::invalid_argument("graph routing requires dimension == 1");
    }
    if (this->getRoutingAlg() == RoutingType::RECONFIGURABLE_GRAPH_TOPO &&
        this->reconfig_file_path_.empty()) {
        throw std::invalid_argument("reconfig_file is required for reconfigurable graph routing");
    }
    if ((this->getRoutingAlg() == RoutingType::CHIPLET_ROUTING_MESH ||
         this->getRoutingAlg() == RoutingType::CHIPLET_STAR_TOPO_ROUTING) &&
        this->cube_number_ != 4) {
        throw std::invalid_argument("chiplet routing requires dimension == 4");
    }
    if (this->physical_port_number_ * this->virtual_channel_number_ > 64) {
        throw std::invalid_argument("physical ports multiplied by vc_cnt must not exceed 64");
    }
}

...
void Flit::setCreditReturn(const AddrType& address, long port) {
    credit_return_address_ = address;
    credit_return_port_ = port;
}

const AddrType& Flit::getCreditReturnAddress() const noexcept {
    return credit_return_address_;
}

long Flit::getCreditReturnPort() const noexcept {
    return credit_return_port_;
}
 ...
 */
Flit::Flit()
    : flit_id_(), flit_type_(FlitType::HEADER), src_addr_(), des_addr_(), start_time_(),
      send_finish_time_(), finish_time_(), data_(), packet_id_(), credit_return_address_() {}
 ...
 */
Flit::Flit(long flit_id, FlitType flit_type, const AddrType& src_addr, const AddrType& des_addr,
           TimeType start_time, DataType data, TPacketId packet_id)
    : flit_id_(flit_id), flit_type_(flit_type), src_addr_(src_addr), des_addr_(des_addr),
      start_time_(start_time), send_finish_time_(), finish_time_(), data_(std::move(data)),
      packet_id_(packet_id), credit_return_address_() {}

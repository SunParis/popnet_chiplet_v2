 ...
 */
template <typename PM, typename V> inline double routing_delay(PM& propertyMap, V& v) {
 ...
 */
template <typename PM, typename E> inline double link_delay(PM& propertyMap, E& e) {
 ...
 */
inline void setDoubleHop(ShortestPath::TNextHopTable& table, std::size_t x, std::size_t y,
 ...
 */
inline void setDoubleValue(ShortestPath::TMatrix& delayTable, std::size_t x, std::size_t y,
                           double delay) {
...
void ShortestPath::calculateShortestPathTables(GraphLib::graph_t& g,
    ...
    std::for_each(edgeList.first, edgeList.second, [&](const GraphLib::edge_t& e) {
        ...
        // Assume that the router forwarding takes time (including the routers of the sender and
        // receiver)
        setDoubleValue(delayTable, src_index, tar_index,
                       link_delay(delay, e) + routerDelay[src_index] + routerDelay[tar_index]);
        setDoubleHop(nextHop, src_index, tar_index, tar_index, src_index);
        setDoubleValue(energyTable, src_index, tar_index,
                       forwardEnergy[src_index] + hopEnergyMap[e] + forwardEnergy[tar_index]);
    });

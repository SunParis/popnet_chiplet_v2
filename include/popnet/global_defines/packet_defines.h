 ...
 * @note The maximum size is preset, and if exceeded, the packet must be fragmented for
 * transmission.
 */
class Flit {
  ...
  private:
    ...
    AddrType credit_return_address_;
    long credit_return_port_{0};
  ...
  public:
    ...
    void setCreditReturn(const AddrType& address, long port);
    const AddrType& getCreditReturnAddress() const noexcept;
    long getCreditReturnPort() const noexcept;
     ...
     */
    Flit(long flit_id_, FlitType flit_type, const AddrType& sor_addr, const AddrType& des_addr,
     ...
     */
    Flit(const Flit&) = default;
    Flit(Flit&&) noexcept = default;
    Flit& operator=(const Flit&) = default;
    Flit& operator=(Flit&&) noexcept = default;
};

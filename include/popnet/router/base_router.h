...
class BaseRouter;
using RouterList = std::vector<std::unique_ptr<BaseRouter>>;
...
class BaseRouter {
  ...
  protected:
    ...
    std::vector<VCType> available_vcs_;
    std::vector<std::vector<VCType>> vc_requests_;
    std::vector<std::vector<VCType>> switch_requests_;
    std::vector<long> input_vc_candidates_;
     ...
     */
    virtual void routingAlg(const AddrType& destination, const AddrType& source, long s_ph,
                            long s_vc);
    ...
    void chiplet_star_topo_routing_alg(const AddrType& des_t, const AddrType& src_t, long s_ph,
                                       long s_vc);
  ...
  public:
     ...
     */
    BaseRouter(const Config& config, AddrType addr, RouterList& router_list, SimulationState& state,
               InputTrace& input_trace, DelayWriter& delay_writer, long physical_port_count = -1);
 ...
 */
inline bool isCentral(const AddrType& address, long centerXY) {
